...
def make_pipeline():
    ...
    pipeline = [
        {"$match": {"country": "India"}},
        {"$unwind": "$isPartOf"},
        {"$group": {"_id": "$isPartOf", "avg_per_region": {"$avg": "$population"}}},
        {"$group": {"_id": "India Regional City Population Average", "avg": {"$avg": "$avg_per_region"}}}
    ]
    return pipeline
...
if __name__ == '__main__':
    ...
    result = [doc for doc in result] #converting iterator to a list
    pprint.pprint(result)
    assert len(result) == 1
    assert result[0]["avg"] == 196025.97814809752