...
API_KEY = { "popular": "a8140d7a230f657610a93ef1d6db9d3f:18:73148962",
            "article": "5c014bd667d47f70d5a93f1e34d603df:14:73148962"}
...
def article_overview(kind, period):
    ...
    #list of dictionaries, where the dictionary key is "section" and value is "title"
    titles = [{news["section"]: news["title"]} for news in data]
    #list of URLs for all media entries with "format": "Standard Thumbnail"
    #get the medias
    medias = [news["media"] for news in data if type(news["media"])==list]
    #flatten the list
    medias = reduce(lambda x,y:x+y, medias)
    #get the metadatas
    metadatas = reduce(lambda x,y:x+y, [media["media-metadata"] for media in medias])
    #filter only the ones which are Standard Thumbnails
    urls = [metadata["url"] for metadata in metadatas if metadata["format"]=="Standard Thumbnail"]
