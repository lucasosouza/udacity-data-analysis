...
def fix_area(area):
    if area == 'NULL' or area == '': return None
    try:
        return float(area)
    except:
        return max_precision(map(lambda x:float(x), area[1:-1].split("|")))

def max_precision(floats):
    precision = map(lambda x:len(str(x/10e9).split('.')[1]), floats)
    return floats[precision.index(max(precision))]
...
def process_file(filename):
    ...
    with open(filename, "r") as f:
        ...
        for idx, line in enumerate(reader):
            ...
            if "areaLand" in line:
                print "{}: {}".format(idx, line["areaLand"])
