...
def make_pipeline():
    ...
    pipeline = [
        {"$unwind": "$isPartOf"},
        {"$group":{"_id": "$isPartOf", "country":{"$first": "$country"}, "avg_region": {"$avg": "$population"}}},
        {"$group":{"_id": "$country", "avgRegionalPopulation": {"$avg": "$avg_region"}}}
    ]
    """
    pipeline = [
        {"$unwind": "$isPartOf"},
        {"$group":{"_id": {"isPartOf": "$isPartOf", "country": "$country"}, "avg_region": {"$avg": "$population"}}},
        {"$group":{"_id": "$_id.country", "avgRegionalPopulation": {"$avg": "$avg_region"}}}
    ]
    return pipeline

    #24367.091472262306-14750.794 < 

"""
Extrapolating from an earlier exercise in this lesson, find the average regional city population 
for all countries in the cities collection. 
average for region
then average for country
"""
...
if __name__ == '__main__':
    ...
    assert result[0]["_id"] == 'Kuwait'
    #assert result[1]["avgRegionalPopulation"] == 363945.0
    assert result[0] == {'_id': 'Kuwait', 'avgRegionalPopulation': 115945.66666666667}
